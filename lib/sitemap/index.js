...
const SOURCE =
  ...
  path.join(
    process.cwd(),
    'pages',
    '/**/!(_document|_error|confirm|search|user|edit|images|show|password_reset)*.js'
  )
const DESTINATION =
...
function buildSitemap() {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const content = fs.readFileSync(path.join(process.cwd(), 'server.js'), {
        encoding: 'utf8'
      })
      ...
      listings.data.listings.forEach((listing) => {
        ...
        const listingUrl = `${SITE_ROOT}/imoveis/${listing.id}`
      ...
      resolve(`Wrote sitemap for ${diskPages.length} pages to ${DESTINATION}`)
    } catch (e) {
      reject(e)
    }
  })
